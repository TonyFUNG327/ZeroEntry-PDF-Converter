...
def detect_bank(pdf_path):
    ...
    if "DBS Bank (Hong Kong) Limited" in text:
        return "DBS"
    if "中國工商銀行（亞洲）" in text or "INDUSTRIAL AND COMMERCIAL BANK OF CHINA (ASIA)" in text:
        return "ICBC_ASIA"
    ...
    if (
        ...
        or "NCB" in text and ("賬戶交易詳情" in text or "Account Transaction Details" in text)
...
def convert_one(pdf_path, output_dir):
    ...
    if bank == "HSBC":
        ...
        accounts = icbc_asia_pdf_converter.extract_pdf(pdf_path)
        report = icbc_asia_pdf_converter.validate_accounts(accounts)
        output_path = icbc_asia_pdf_converter.write_workbook(accounts, output_dir / f"{pdf_path.stem}.xlsx")
    elif bank == "COMM":
