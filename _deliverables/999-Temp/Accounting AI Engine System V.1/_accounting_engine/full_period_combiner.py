...
def build_combined_payload(opening_payload: dict[str, Any], bank_payload: dict[str, Any], report_path: Path) -> dict[str, Any]:
    ...
    for line in bank_payload["journal_lines"]:
        account_id = text(line["account_id"])
        account_names.setdefault(account_id, text(line["account_description"]))
        journal_lines.append(
            {
                "source": "Bank",
                "date": line["date"],
                ...
                "jrnl": "GENJ",
                "account_id": account_id,
                "account_description": text(line["account_description"]),
                "description": text(line["description"]),
                "debit": number(line["debit"]),
                "credit": number(line["credit"]),
                "signed_amount": number(line["signed_amount"]),
            }
        )

    totals = defaultdict(float)
    period_activity = defaultdict(float)
    for line in journal_lines:
        totals[line["account_id"]] += number(line["signed_amount"])
        if line["source"] != "Opening":
            period_activity[line["account_id"]] += number(line["signed_amount"])

    sample_tb = parse_sample_tb(report_path)
    manual_jv = parse_sample_manual_jv(report_path)
    ...
    gl_rows = sorted(journal_lines, key=lambda x: (sort_key(x["account_id"]), x["date"], x["reference"], x["credit"]))
    ...
    return {
        "summary": {
            "source": "Opening balances + clean bank record journals",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Combine opening balances and bank journals for a full-period accounting run.")
    parser.add_argument("--opening-json", type=Path, required=True)
    parser.add_argument("--bank-json", type=Path, required=True)
    parser.add_argument("--sample-report", type=Path, required=True)
    parser.add_argument("--out-json", type=Path, required=True)
    args = parser.parse_args()

    payload = build_combined_payload(
        load_json(args.opening_json),
        load_json(args.bank_json),
        args.sample_report,
    )
