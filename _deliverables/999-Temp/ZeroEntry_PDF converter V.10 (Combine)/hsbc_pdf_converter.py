...
CURRENCIES = {"HKD", "USD", "CNY", "JPY", "EUR", "GBP", "AUD", "CAD", "CHF", "NZD", "SGD"}
MONTHS = {
    "Jan": 1,
    "January": 1,
    "Feb": 2,
...
def normalize_date(day, month, statement_year=2025, statement_month=4):
...
def detect_statement_date(text):
    match = re.match(r"^\s*(\d{1,2})\s+([A-Z][a-z]+)\s+(\d{4})\s*$", text)
    if not match:
        return None
    month = match.group(2)
    if month not in MONTHS:
        return None
    return int(match.group(3)), MONTHS[month]


def account_name_for_foreign_currency(currency):
    return f"HSBC {currency} Foreign Currency Savings"


def sheet_title_for_account(account):
    match = re.match(r"^HSBC ([A-Z]{3}) Foreign Currency Savings$", account)
    if match:
        return f"HSBC {match.group(1)} FCY Savings"
    return account[:31]
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page in pdf.pages[:3]:
            for text_line in (page.extract_text() or "").splitlines():
                detected = detect_statement_date(text_line)
                if detected:
                    statement_year, statement_month = detected
                    statement_date_found = True
                    break
            if statement_date_found:
                break
        ...
        for page in pdf.pages:
            ...
            for line in lines:
                ...
                detected = detect_statement_date(text)
                if detected:
                    statement_year, statement_month = detected

                if text.strip() == "HSBC Business Direct HKD Current":
                    current_account = "HSBC HKD Current Account"
                    current_currency = None
                    in_foreign_table = False
                    in_table = False
                    current_date = None
                    pending_desc = []
                    continue
                if text.strip() == "HSBC Sprint Account HKD Current":
                    current_account = "HSBC HKD Current Account"
                    current_currency = None
                    in_foreign_table = False
                    in_table = False
                    ...
                    pending_desc = []
                    continue
                if text.strip() == "HSBC Sprint Account HKD Savings":
                    current_account = "HSBC HKD Savings Account"
                    current_currency = None
                    in_foreign_table = False
                    in_table = False
                    current_date = None
                    pending_desc = []
                    continue
                if text.strip() == "HSBC Business Direct HKD Savings":
                    current_account = "HSBC HKD Savings Account"
                    current_currency = None
                    in_foreign_table = False
                    in_table = False
                    ...
                    pending_desc = []
                    continue
                if text.strip() == "HSBC Business Direct Foreign Currency Savings":
                    current_account = None
                    current_currency = None
                    ...
                    pending_desc = []
                    continue
                if text.startswith("Date Transaction"):
                    in_table = current_account is not None
                    pending_desc = []
                    continue
                if text.startswith("CCY Date Transaction"):
                    in_table = in_foreign_table
                    pending_desc = []
                    continue
                if text.startswith("Total No. of") or text.startswith("TotalNo.of") or text.startswith("Total Deposit Amount") or text.startswith("TotalDepositAmount"):
                    in_table = False
                    in_foreign_table = False
                    pending_desc = []
                    continue
                if not in_table:
                    continue
                if text.startswith("The Hongkong and Shanghai Banking Corporation"):
                    continue

                if in_foreign_table:
                    if words and words[0]["text"] in CURRENCIES and words[0]["x0"] < 76:
                        current_currency = words[0]["text"]
                        current_account = account_name_for_foreign_currency(current_currency)

                    date_words = [word for word in words if 75 <= word["x0"] <= 105]
                    ...
                    date_words = [word for word in words if 55 <= word["x0"] <= 85]
                    if len(date_words) >= 2 and date_words[0]["text"].isdigit() and date_words[1]["text"] in MONTHS:
                        current_date = normalize_date(
                            date_words[0]["text"],
                            date_words[1]["text"],
                ...
                amounts = {}
                desc_words = []
                has_balance_dr = any(word["text"] == "DR" and word["x0"] >= 525 for word in words)
                for word in words:
                    ...
                    if cls:
                        ...
                        if cls == "Balance" and has_balance_dr:
                            value = -abs(value)
                        amounts[cls] = value
                    elif (113 if in_foreign_table else 96) <= word["x0"] <= 300:
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Convert HSBC Business Direct statement PDF to Excel.")
    parser.add_argument(
        "input",
        type=Path,
        nargs="?",
