...
def account_name(currency, account_type):
    normalized = " ".join(account_type.replace("§", "").split())
    upper = normalized.upper()
    if "MULTI-CURRENCY" in upper:
        normalized = "Multi-Currency Savings Account"
    elif "往來" in normalized or "CURRENT" in upper:
        normalized = "Current Account"
    elif "儲蓄" in normalized or "SAVINGS" in upper:
        normalized = "Savings Account"
    return f"NCB {currency} {normalized}"


def sheet_title_for_account(account):
    if "Multi-Currency Savings Account" in account:
        return account.replace("Multi-Currency", "MCY")[:31]
    return account[:31]
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            for _y, words in group_words_by_line(page):
                ...
                if "賬戶交易詳情" in text or "Account Transaction Details" in text:
                ...
                if is_footer_or_notice(text):
                    ...
                    continue
                if (
                    text in {"往來賬戶", "Deposit", "Savings Account", "Current Account"}
                    or "交易日期" in text and "交易摘要" in text
                    or "Transaction Date" in text and "Transaction Details" in text
                    or text in {"Currency", "Balance in Original Currency", "in Original Currency"}
                    or text.startswith("Balance/(Outstanding")
                ):
                    continue
