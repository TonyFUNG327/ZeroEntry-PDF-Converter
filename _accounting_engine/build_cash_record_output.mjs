...
if (!inputJson || !outputXlsx) {
  throw new Error("Usage: node build_cash_record_output.mjs --input-json payload.json --output-xlsx output.xlsx");
}
...
function accountTotals(lines) {
  const buckets = new Map();
  for (const line of lines || []) {
    const accountId = asText(line.account_id);
    const current = buckets.get(accountId) || {
      account_id: accountId,
      account_description: asText(line.account_description),
      debit: 0,
      credit: 0,
      signed_amount: 0,
    };
    ...
    current.signed_amount += Number(line.signed_amount || 0);
    buckets.set(accountId, current);
  }
  return [...buckets.values()].sort((a, b) => a.account_id.localeCompare(b.account_id, undefined, { numeric: true }));
}

const summary = workbook.worksheets.add("Summary");
summary.showGridLines = false;
summary.getRange("A1:I1").merge();
summary.getRange("A1").values = [["Cash Record Import Summary"]];
...
const summaryRows = [
  ...
  ["Cash Sheets Imported", s.sheet_count],
...
const journal = workbook.worksheets.add("Cash Journal Lines");
...
const accountRows = accountTotals(payload.journal_lines).map((row) => [
